...
class _TransparencyGridPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Apply the offset.
    final double dx = offset.dx % (squareSize * 2);
    final double dy = offset.dy % (squareSize * 2);

    // Calculate the number of squares in each direction.
    final int xMax = (size.width / squareSize).ceil();
    final int yMax = (size.height / squareSize).ceil();
